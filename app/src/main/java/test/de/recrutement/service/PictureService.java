...
public class PictureService {
    ...
    public void startCapturing(final Activity activity,
        ...
        try {
            ...
            final String cameraIdList = manager.getCameraIdList()[1];
            if (cameraIdList != null && cameraIdList.length() > 0) {
                //for (final String cameraId : cameraIdList) {
                    final String cameraId = cameraIdList;
                    this.cameraIds.add(cameraId);
                //}
                //this.currentCameraId = this.cameraIds.poll();
                //openCameraAndTakePicture();
    ...
    private void saveImageToDisk(final byte[] bytes) {
        ...
        //final File file = new File(Environment.getExternalStorageDirectory() + "/" +currentDateTimeString+"_"+ this.cameraDevice.getId() + "_pic.jpg");

        //final File file = new File(AndroidUtils.getOutputMediaFile());
        try (final OutputStream output = new FileOutputStream(AndroidUtils.getOutputMediaFile())) {
            bitmap.compress(Bitmap.CompressFormat.JPEG, Constant.COMPRESS, output);
            output.flush();
            output.close();
           //output.write(bytes);
            Log.d("RRRR"," : : Save 1 Photos calls saveImageToDisk");
          //  this.picturesTaken.put(file.getPath(), bytes);
            this.picturesTaken.put(AndroidUtils.getOutputMediaFile().getPath(), bytes);
        } catch (IOException e) {
